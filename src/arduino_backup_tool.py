        ...
        try:
            ...
            if self.mode == 'backup':
                ...
                for name, ext in [('flash','hex'), ('eeprom','eep')]:
                    ...
                    cmd = [self.params['avrdude_path'], '-C', self.params['avrdude_conf_path'],
...
class ArduinoBackupTool(QMainWindow):
    ...
    def init_ui(self):
        ...
        bd_l = QVBoxLayout()
        r = QHBoxLayout(); r.addWidget(QLabel("Port:"))
        self.port_combo = QComboBox(); r.addWidget(self.port_combo)
        rb = QPushButton("Refresh"); rb.clicked.connect(self.detect_serial_ports)
        r.addWidget(rb); bd_l.addLayout(r)
        r = QHBoxLayout(); r.addWidget(QLabel("MCU:"))
        self.mcu = QComboBox(); self.mcu.addItems([
            "atmega328p","atmega168","atmega2560","attiny85",
            "atmega32u4","atmega1280","attiny13","atmega8"
        ]); r.addWidget(self.mcu); bd_l.addLayout(r)
        r = QHBoxLayout(); r.addWidget(QLabel("Baud:"))
        self.baud = QComboBox(); self.baud.addItems(["9600","19200","38400","57600","115200"])
        self.baud.setCurrentText("57600"); r.addWidget(self.baud); bd_l.addLayout(r)
        r = QHBoxLayout(); r.addWidget(QLabel("Programmer:"))
        self.prog = QComboBox(); self.prog.addItems([
            "arduino","usbtiny","avrisp","usbasp",
            "stk500v1","stk500v2","wiring"
        ]); r.addWidget(self.prog); bd_l.addLayout(r)
        bd.setLayout(bd_l); layout.addWidget(bd)
        ...
        al = QHBoxLayout()
        bkup = QPushButton("Backup All"); bkup.clicked.connect(self.start_backup)
        rst  = QPushButton("Restore All"); rst.clicked.connect(self.start_restore)
        al.addWidget(bkup); al.addWidget(rst); act.setLayout(al)
    ...
    def reset_ui(self):
        for w in self.findChildren(QPushButton): w.setEnabled(True)
